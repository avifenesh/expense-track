  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.process.stdout?.on('data', (data) => {
        ...
        this.startupLog += output
        // Always log in CI for debugging; respect DEBUG_BACKEND for local
        if (process.env.CI || process.env.DEBUG_BACKEND) {
      ...
      this.process.stderr?.on('data', (data) => {
        const output = data.toString()
        this.startupLog += output
        // Always log errors in CI for debugging; respect DEBUG_BACKEND for local
        if (process.env.CI || process.env.DEBUG_BACKEND) {
      ...
      this.process.on('exit', (code) => {
        if (code !== 0 && code !== null) {
          ...
          console.error('[BackendManager] Startup log:', this.startupLog)
        }
        this.process = null
      })
  ...
  private async waitForHealthy(): Promise<void> {
    ...
    while (Date.now() - startTime < STARTUP_TIMEOUT) {
      try {
        ...
        if (response.status === 200 || response.status === 401 || response.status === 403) {
          console.log(`[BackendManager] Server healthy (status: ${response.status}, ${Date.now() - startTime}ms)`)
          return
        }
      } catch {
    ...
    throw new Error(`Backend failed to start within ${STARTUP_TIMEOUT}ms. Startup log:\n${this.startupLog}`)
  }
