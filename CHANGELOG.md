...
- **Mobile E2E Tests**: Fixed 5 test failures causing CI instability (#333)
  - Added `testID` prop to `TransactionListItem` component for reliable E2E targeting
  - Updated `DashboardScreen` to pass `testID` to transaction list items with unique identifiers
  - Increased default timeouts for CI stability (SHORT: 5s, MEDIUM: 10s, LONG: 60s, STARTUP: 120s)
  - Enhanced `performLogin` helper with comprehensive error handling and detailed logging
  - Added startup log capture to `BackendManager` for better debugging of server startup failures
