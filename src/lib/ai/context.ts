...
export async function buildFinancialContext(
  ...
  const selectedAccountName = accounts.find((a) => a.id === accountId)?.name || 'All Accounts'
  ...
  const currencySymbol = preferredCurrency === Currency.EUR ? '€' : preferredCurrency === Currency.ILS ? '₪' : '$'
