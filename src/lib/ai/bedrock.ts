...
/**
 * Validate that the requested accountId is in the allowed list
 * Returns the accountId if valid, or the first allowed account if not specified
 */
function validateAccountAccess(
  ...
  if (!requestedAccountId) {
    // Default to first allowed account
    return allowedAccountIds[0]
  }
  if (!allowedAccountIds.includes(requestedAccountId)) {
    throw new Error(`Access denied: Cannot access account in ${context}`)
  }
  return requestedAccountId
}

/**
 * Create account-scoped AI tools
 * @param allowedAccountIds - List of account IDs the user can access
 */
export function createTools(allowedAccountIds: string[]): Record<string, ToolConfig> {
  return {
    get_transactions: createTool({
      ...
      execute: async ({ monthKey, accountId }: { monthKey: string; accountId?: string }) => {
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'get_transactions')
    ...
    search_transactions: createTool({
      ...
      execute: async (args: {
        ...
        // Scope to allowed accounts (security: prevent cross-account data access)
        const validAccountId = args.accountId
          ? validateAccountAccess(args.accountId, allowedAccountIds, 'search_transactions')
          : undefined
        if (validAccountId) {
          where.accountId = validAccountId
        } else {
          // If no specific account, filter to all allowed accounts
          where.accountId = { in: allowedAccountIds }
        }
    ...
    recommend_savings: createTool({
      ...
      execute: async ({
        ...
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'recommend_savings')
    ...
    get_budgets: createTool({
      ...
      execute: async ({ monthKey, accountId }: { monthKey: string; accountId?: string }) => {
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'get_budgets')
        const data = await getDashboardData({ accountId: validAccountId, monthKey })
        return data.budgets
      },
    }),
    ...
    get_holdings: createTool({
      ...
      execute: async ({
        ...
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'get_holdings')
    ...
    analyze_holdings_performance: createTool({
      ...
      execute: async ({
        ...
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'analyze_holdings_performance')
        const { getHoldingsWithPrices } = await import('@/lib/finance')
    ...
    forecast_cashflow: createTool({
      ...
      execute: async ({
        ...
        const validAccountId = validateAccountAccess(accountId, allowedAccountIds, 'forecast_cashflow')
        const data = await getDashboardData({
          accountId: validAccountId,
          monthKey,
          preferredCurrency: preferredCurrency as Currency | undefined,
    ...
    get_stock_price: createTool({
      ...
      execute: async ({ symbol }: { symbol: string }) => {
        ...
        const result = await getStockPrice(symbol.toUpperCase())
        if (!result.success) {
...
/**
 * @deprecated Use createTools(allowedAccountIds) for account-scoped tools
 * Kept for backward compatibility - uses empty allowedAccountIds (all validation fails)
 */
export const tools: Record<string, ToolConfig> = createTools([])
