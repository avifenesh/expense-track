...
export const fastModel = bedrock(
...
export const tools: Record<string, ToolConfig> = {
  get_transactions: createTool({
    ...
    parameters: z.object({
      ...
      accountId: z
        .string()
        .optional()
        .describe("Optional account ID to filter by"),
    }),
    execute: async ({
      monthKey,
      accountId,
  ...
  search_transactions: createTool({
    description:
      "Search transactions with flexible filters (date range, category, text, amount).",
    parameters: z.object({
      accountId: z.string().optional(),
      monthKey: z
        .string()
        .optional()
        .describe("Base month YYYY-MM for default range"),
      ...
      text: z
        .string()
        .optional()
        .describe("Search in description/category/account"),
    ...
    execute: async (args: {
      ...
      const filtered = results.filter((t) => {
        ...
        if (typeof args.minAmount === "number" && amt < args.minAmount)
          return false;
        if (typeof args.maxAmount === "number" && amt > args.maxAmount)
          return false;
        return true;
      });
  ...
  create_transaction: createTool({
    ...
    parameters: z.object({
      ...
      isRecurring: z
        .boolean()
        .optional()
        .describe("Whether this is part of a recurring template"),
      recurringTemplateId: z
        .string()
        .optional()
        .describe("Optional recurring template ID"),
    }),
    execute: async (args: {
      accountId: string;
      categoryId: string;
  ...
  update_transaction: createTool({
    description:
      "Update an existing transaction. Provide all fields along with the id.",
    parameters: z.object({
  ...
  create_holding: createTool({
    description:
      "Add a new holding (stock/ETF/savings). Category must be marked as holding.",
    parameters: z.object({
      accountId: z.string(),
      categoryId: z.string(),
  ...
  update_holding: createTool({
    description:
      "Update an existing holding by id (quantity, averageCost, notes).",
    parameters: z.object({
      id: z.string(),
      quantity: z.number(),
      averageCost: z.number(),
  ...
  recommend_savings: createTool({
    description:
      "Analyze this month and recommend concrete next steps to save money.",
    parameters: z.object({
    ...
    execute: async ({
      ...
      if (overspends.length > 0) {
        const catList = overspends
          .map((o) => `${o.categoryName} (${o.overBy})`)
          .join(", ");
        suggestions.push({
          title: "Reduce overspending in top categories",
          action: `Set temporary caps or pause discretionary spend in: ${catList}.`,
          rationale:
            "These categories exceeded their planned budgets this month.",
        });
      }
      ...
      if (topSpend.length > 0) {
        ...
        suggestions.push({
          ...
          rationale:
            "Small reductions in the largest categories yield meaningful savings.",
        });
      }
      ...
      if (trendingWorse) {
        suggestions.push({
          ...
          action:
            "Delay non-essential purchases until the next month and review recurring commitments.",
      ...
      if (data.recurringTemplates.length > 0) {
        suggestions.push({
          ...
          action:
            "Review active recurring charges; cancel or downsize low-value subscriptions for 1-2 months.",
          rationale:
            "Recurring items often accumulate unnoticed and are easy to pause short term.",
        });
      }

      const summary = `Based on ${monthKey}, you can likely save by focusing on ${
        overspends[0]?.categoryName ??
        topSpend[0]?.name ??
  ...
  get_budgets: createTool({
    description:
      "Get planned vs actual budgets for a month (and optional account).",
    parameters: z.object({
  ...
  apply_recurring_templates: createTool({
    description:
      "Apply active recurring templates into a given month (optionally subset by template IDs).",
    parameters: z.object({
      monthKey: z.string().describe("YYYY-MM"),
      accountId: z.string(),
  ...
  analyze_holdings_performance: createTool({
    description:
      "Summarize winners/losers and concentration risks in holdings.",
    parameters: z.object({
      accountId: z.string().optional(),
      preferredCurrency: currencyEnum.optional(),
    }),
    execute: async ({
      ...
      const totalMV = holdings.reduce(
        (acc, h) => acc + (h.marketValueConverted ?? h.marketValue),
        0
      );
      const concentration = holdings
        ...
        .sort(
          (a, b) =>
            (b.marketValueConverted ?? b.marketValue) -
            (a.marketValueConverted ?? a.marketValue)
        )
        .slice(0, 5)
        .map((h) => ({
          symbol: h.symbol,
          weight: Number(
            (
              ((h.marketValueConverted ?? h.marketValue) / (totalMV || 1)) *
              100
            ).toFixed(2)
          ),
        }));
  ...
  refresh_exchange_rates: createTool({
    description:
      "Refresh cached exchange rates before running currency-sensitive analysis.",
    parameters: z.object({}),
    execute: async () => {
  ...
  forecast_cashflow: createTool({
    description:
      "Forecast next 1–3 months net using recent history and budgets.",
    parameters: z.object({
      accountId: z.string().optional(),
    ...
    execute: async ({
      ...
      const avgNet = last.length
        ? last.reduce((acc, p) => acc + p.net, 0) / last.length
        : 0;

      const budgetsNet = (() => {
        const income = data.budgets
          .filter((b) => b.categoryType === "INCOME")
          .reduce((acc, b) => acc + b.planned, 0);
        const expense = data.budgets
          .filter((b) => b.categoryType === "EXPENSE")
          .reduce((acc, b) => acc + b.planned, 0);
  ...
  get_stock_price: createTool({
    description:
      "Get the latest cached price and freshness for a stock/ETF symbol (USD).",
    parameters: z.object({
  ...
  refresh_stock_prices: createTool({
    description:
      "Refresh and cache prices for a list of symbols (Alpha Vantage, rate-limited).",
    parameters: z.object({
  ...
  get_exchange_rate: createTool({
    description:
      "Get exchange rate from one currency to another (uses cache or fetch).",
    parameters: z.object({
      from: currencyEnum,
      to: currencyEnum,
    }),
  ...
  get_exchange_rates: createTool({
    description:
      "Fetch the latest exchange rates for a base currency (Frankfurter).",
    parameters: z.object({
      base: currencyEnum,
    }),
