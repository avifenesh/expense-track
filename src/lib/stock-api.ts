...
/**
 * Check if stock prices need refreshing based on last update time
 */
export async function needsRefresh(symbols: string[]): Promise<boolean> {
  ...
  const hoursSince = (Date.now() - mostRecent.fetchedAt.getTime()) / (1000 * 60 * 60)
  return hoursSince > STOCK_PRICE_MAX_AGE_HOURS
}

/**
 * Auto-refresh logic for serverless cold starts (non-blocking)
 */
export async function maybeRefreshOnStartup(symbols: string[]): Promise<void> {
  if (symbols.length === 0) return

  try {
    const shouldRefresh = await needsRefresh(symbols)

    if (shouldRefresh) {
      // Non-blocking background refresh
      refreshStockPrices(symbols).catch((error) => {
        console.error('Background stock price refresh failed:', error)
      })
    }
  } catch (error) {
    console.error('maybeRefreshOnStartup error:', error)
  }
}
